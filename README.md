...
прописывает сервер сам, Codex — отдельной командой; и сразу оговорка: на живом
Codex это не проверялось никем, подробности в разделе
...
2. установленный **один из трёх: Claude Desktop, Claude Code или Codex**.
   ...
   отдельной командой `garant register --codex`, и на живом Codex это никем
   не проверялось — подробности в разделе
   [«Codex (подписка ChatGPT)»](#codex-подписка-chatgpt);
...
и перезапуска они должны появиться.
...
Она дописывает в файл настроек одну таблицу:
...
В `command` стоит имя команды, а не путь к ней: путь устарел бы с первым же
