...
**Нужен Claude Desktop или Claude Code.** Это программы, которые умеют
запускать MCP-сервер на вашей машине. Установщик прописывает сервер только
им двоим.

**ChatGPT не подойдёт.** Он не запускает локальные MCP-серверы, и инструменты
«Гаранта» в нём не появятся — ни в браузере, ни в приложении. Другие
MCP-клиенты, умеющие запускать локальную программу, работать могут, но запись
в их конфигурацию придётся вписать руками: `garant register` знает только
Claude Desktop, Claude Code и `.mcp.json` конкретного проекта.
...
1. действующая подписка «Гаранта» — логин и пароль, которыми вы входите
   в обычном браузере;
2. установленный Claude Desktop или Claude Code;
...
регистрация у Claude Desktop и Claude Code, автозапуск, отпечаток браузера.
**На каждый красный пункт печатается точная команда, которой он чинится** —
её можно скопировать и выполнить. Красный пункт без команды починки в этом
пульте невозможен: он падает как дефект самого пульта.
...
`doctor` показывает состояние регистрации у Claude Desktop и Claude Code
отдельными пунктами. После него перезапустите Claude (шаг 3). Запись, которую
пишет `garant register`, — одна строка `{"command": "garant-mcp"}`; всё
остальное в конфигурации клиента остаётся нетронутым, а рядом кладётся копия
`.bak`.
...
garant unregister      снять эту запись
